...
# NOTE: ``consolidate_descriptions`` lived here previously. It scanned
# the per-doc extraction batch for fragmented descriptions and called
# the LLM to merge them — but the per-doc batch almost never has the
# repeat-mention pattern that triggers fragmentation (that happens
# across documents). Replaced by ``graph.summarize.summarize_descriptions``
# called from the pipeline's post-upsert ``_summarize_phase`` which
# reads the cumulative graph state where the bloat actually lives.
