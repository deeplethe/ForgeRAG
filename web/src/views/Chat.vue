...
/* Markdown body rules (h1/ul/code/etc.) live in
   ``components/AgentMessageBody.vue`` — that's where ``.msg-body``
   actually exists, so the ``:deep(...)`` selectors line up with the
   scoped data-attribute. Keeping them here previously meant Vue
   compiled them to ``.msg-body[data-v-chat] *``, which never
   matched anything (the ``.msg-body`` element is owned by the
   child component's scope, not this one), so headings inherited
   Tailwind preflight's no-op typography and ``<ul>``s lost their
   bullets. */
