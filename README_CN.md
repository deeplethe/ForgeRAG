...
> ## 📦 v1.0.0 是最后一个 OSS 版本
>
> 这是 OpenCraig 在 AGPLv3 协议下最后一个开源版本。后续开发以
> **OpenCraig Enterprise（v3.0+）** 商业版形式继续——独立产品，
> 包含血缘、审计、产物提升回库、可审计的团队工作流、托管沙箱执行
> 等。**OSS 版（本仓库）功能完整、可永久免费自托管**。
>
> 安全补丁通过 GitHub issue 接受，**12 个月**（截至 2027-05-09），
> 之后仓库归档。tag 之后不再接受功能 PR。
>
> 简介：知识管理 + 智能检索（带结构化引用）。多用户、文件夹级授权、
> BYOK LLM。可生产级地服务一个小团队的研究工作流。
...
```mermaid
flowchart LR
    Q([❓ 提问]) --> AGENT[🤖 Agent 循环<br/>Hermes Agent runtime]
...
**Agent 驱动的检索，不是固定流水线**。Hermes Agent runtime（in-process）按问题决定调哪些工具、什么顺序。"3.2 节讲了什么"可能只调 read_tree + read_chunk；"X 和 Y 在整个语料里怎么关联"先用 graph_explore。多跳问题在多轮里链多个工具。
...
flowchart TB
    ...
    subgraph "后端 (FastAPI · Python 3.13)"
        ...
        TOOLS[工具<br/>search_vector · graph_explore<br/>read_chunk · read_tree · web_search<br/>rerank · import_from_library]
...
v1.0 之后 OSS 仓库**只接受安全补丁**（直至 2027-05-09）。无新功能、无架构变更、无功能性 PR。鼓励基于此版本在 AGPLv3 下 fork。

---

## 🎁 版本

| | **OpenCraig OSS v1.0.0**（本仓库） | **OpenCraig Enterprise v3.0+** |
|---|---|---|
| 协议 | AGPLv3 | 商业，详谈 |
| 源码 | 开源，本仓库 | 闭源 |
| 自托管 | ✅ 永久免费 | ✅ 提供 |
...
| Skills（团队工作流） | ❌ | ✅ |
| SSO / SCIM | ❌ | ✅ |
| 支持 | GitHub issues，12 月安全补丁 | SLA，专人支持 |
| 路线图 | v1.0.0 后冻结 | 持续开发 |
...
**v1.0.0 是最后一个 OSS 版本**——tag 后不再接受功能 PR。仍接受的：

- **安全报告** 通过 GitHub issues（私密漏洞披露优先——见 SECURITY.md）。补丁接受到 **2027-05-09**。
- **文档修复** —— typo、断链、澄清类 PR 在维护窗口内欢迎。
- **翻译** —— README / 设置向导文案的社区翻译。

其他事项（新功能、架构变更、大型重构），请**在 AGPLv3 下自由 fork**——这正是该协议存在的目的。Enterprise 版独立发布，也不接外部贡献。

签过 [CLA](RELICENSING.md#future-contributions) 的过去贡献者列在 CONTRIBUTORS.md。核心保持 AGPLv3。
